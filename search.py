...
def format_time_ago(published_date, reference_time=None):
    if not published_date:
        return "Unknown time"
    try:
        if reference_time is None:
            reference_time = "2025-01-03T14:26:45Z"
        date_formats = [
            "%Y-%m-%dT%H:%M:%S.%fZ",
            "%Y-%m-%dT%H:%M:%SZ",
            "%Y-%m-%d %H:%M:%S",
        ...
        now = datetime.strptime(reference_time, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
...
def perform_exa_search(exa_client, query, num_results=5, hours_back=24, categories=None):
    try:
        ...
        base_params = {
            'query': query,
            'num_results': max(2, num_results // 3),
...
Title & H1 Options (in Thai) under 60 characters. Meta Description Options (in Thai) under 160 characters, both integrating the primary keyword naturally, both are engaging, with news-like sentences, and are click-worthy.
    ...
    if st.session_state.search_results:
        ...
        for idx, result in enumerate(results['results']):
            with st.container():
                ...
                with cols[1]:
                    ...
                    url = result['url']
                    published_date = format_time_ago(result['published_date'], '2025-01-03T14:26:45Z')
                    
                    st.markdown(f'<div class="search-result">{title}<br><span class="search-result-source">Source: {source} | Published: {published_date}</span></div>', unsafe_allow_html=True)
        ...
        if st.session_state.selected_indices or user_main_text.strip():
            news_angle = st.text_input(
                "News Angle",
                value="Breaking News",
                help="Enter the news angle for the article (e.g. Breaking News, Technical Analysis)"
            )
