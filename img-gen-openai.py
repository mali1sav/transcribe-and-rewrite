...
def upload_image_to_wordpress(image_bytes, wp_url, username, wp_app_password,
                              filename="generated_image.jpg", alt_text="Generated Image"):
    ...
    try:
        ...
        if response.status_code in (200, 201):
            ...
            if update_payload:
                ...
                update_response = requests.post(update_endpoint, json=update_payload,
                                                auth=HTTPBasicAuth(username, wp_app_password))
...
def process_image_for_wordpress(image_bytes, target_size=(1200, 800), final_quality=80):
    """
    Force-fit (crop/resize) to exactly 1200 × 800 px — ideal for WP.
    """
    try:
        ...
        # crop + resize while preserving aspect ratio, centred
        img_fitted = ImageOps.fit(img, target_size, Image.Resampling.LANCZOS, centering=(0.5, 0.5))

        jpeg_bytes_io = BytesIO()
        img_fitted.save(jpeg_bytes_io, format="JPEG", quality=final_quality, optimize=True)
...
if not OPENAI_API_KEY:
    ...
    with st.form("openai_image_form"):
        ...
        prompt_text = st.text_area("Enter your image prompt (English only - for AI generation):",
                                   height=80, key="prompt_input")
        alt_text_for_generated_input = st.text_area("Enter Alt text for your generated image:",
                                                    height=80, key="generated_alt_text_input_val")
                ...
                with st.spinner("Generating and processing image with OpenAI..."):
                    try:
                        response = openai_client.images.generate(
                            ...
                            prompt=prompt_text,
                            n=1,
                            size="1536x1024",             # aspect ratio 3:2 matches 1200x800 on downscale
                        ...
                        if response.data and response.data[0].b64_json:
                            ...
                            if final_image_bytes_io:
                                ...
                                st.success("AI image generated, resized to 1200×800 and ready.")
                            else:
...
with st.form("user_image_upload_form"):
    ...
    user_alt_text_widget_input = st.text_area("Enter alt text for your image:", height=80,
                                              key="user_alt_text_input_val")
        ...
        if uploaded_file is not None and user_alt_text_widget_input.strip():
            ...
            with st.spinner("Processing your uploaded image..."):
                processed_user_image_io = process_image_for_wordpress(
                    st.session_state.user_uploaded_raw_bytes)
                if processed_user_image_io:
                    st.session_state.active_image_bytes_io = processed_user_image_io
                    st.session_state.active_image_alt_text = (
                        st.session_state.user_uploaded_alt_text_input_field_value)
                    st.success("Uploaded image processed to 1200×800 and set as active.")
                else:
...
if st.session_state.active_image_bytes_io and st.session_state.active_image_alt_text:
    ...
    st.image(st.session_state.active_image_bytes_io,
             caption=f"ALT Text Preview: {display_caption}")
    ...
    clean_filename_base = "".join(
        c if c.isalnum() or c in (' ', '_') else '' for c in current_alt_text_for_wp_upload[:30]
    ).rstrip().replace(' ', '_')
    upload_filename = (f"{clean_filename_base}_processed.jpg"
                       if clean_filename_base else "processed_image.jpg")
    ...
    for site_key, column in sites_to_upload.items():
        with column:
            if st.button(f"Upload to {site_key.replace('_', ' ').title()}",
                         key=f"upload_{site_key}", use_container_width=True):
                site_config = WP_SITES.get(site_key)
                if site_config and all(site_config.get(k)
                    ...
                    with st.spinner(f"Uploading to {site_key}..."):
                        upload_image_to_wordpress(
                            ...
                            alt_text=current_alt_text_for_wp_upload
                        )
                else:
                    st.error(f"Missing or incomplete configuration for {site_key}. Check .env file.")
elif not st.session_state.active_image_bytes_io:
